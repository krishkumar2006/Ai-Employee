...
# Platinum Tier: cloud mode guard (optional — gracefully absent if config.py missing)
try:
    from config import cfg as _cfg
    _CLOUD_GUARD_AVAILABLE = True
except ImportError:
    _CLOUD_GUARD_AVAILABLE = False


def _assert_not_cloud(action: str) -> str | None:
    """Return an error string if this action is blocked in cloud mode, else None."""
    if not _CLOUD_GUARD_AVAILABLE:
        return None
    try:
        _cfg.assert_allowed(action, "odoo_mcp")
        return None
    except Exception as e:
        return str(e)


# Platinum Tier: DRY_RUN + rate limiting (optional — gracefully absent)
try:
...
def _is_dry_run() -> bool:
    """True when DRY_RUN env var is set; checked before every write operation."""
    if _CLOUD_GUARD_AVAILABLE:
...
def _check_rate_limit(action: str) -> str | None:
    """
    Check + record one use of action against the sliding-window rate limit.
    Returns an error string if the limit is exceeded, else None.
    """
    if not _RATE_LIMITER_AVAILABLE:
        return None
    try:
        remaining = _limiter.check_and_record(action)
        logger.debug("Rate limit '%s': %d remaining this hour", action, remaining)
        return None
    except _RateLimitError as e:
        audit_log.warn(EV_API_FAIL, action=f"rate_limit_{action}", error=str(e))
        return str(e)
...
def odoo_confirm_invoice(draft_id: str) -> str:
    ...
    """
    # Cloud guard — confirms must run on local machine only
    blocked = _assert_not_cloud("odoo_confirm")
    if blocked:
        return f"BLOCKED (cloud mode): {blocked}\nOpen your Odoo URL and confirm the invoice manually in the Odoo UI."
    ...
    # Dry-run guard — log and return without touching Odoo
    if _is_dry_run():
        return (
            f"[DRY RUN] Would create invoice in Odoo:\n"
            f"  Draft ID : {draft_id}\n"
            f"  Partner  : {draft['partner_name']}\n"
            f"  Total    : {format_currency(draft['total'])}\n"
            f"  Date     : {draft['invoice_date']}\n"
            f"\nNo Odoo write performed. Set DRY_RUN=false to execute for real."
        )

    # Rate limit guard — max odoo_write actions per hour
    rate_err = _check_rate_limit("odoo_write")
    if rate_err:
        return f"RATE LIMIT EXCEEDED: {rate_err}"
...
def odoo_confirm_payment(draft_id: str) -> str:
    ...
    blocked = _assert_not_cloud("odoo_confirm")
    if blocked:
        return f"BLOCKED (cloud mode): {blocked}\nOpen your Odoo URL and register the payment manually in the Odoo UI."
    ...
    if _is_dry_run():
        return (
            f"[DRY RUN] Would register payment in Odoo:\n"
            f"  Draft ID : {draft_id}\n"
            f"  Partner  : {draft.get('partner_name', '?')}\n"
            f"  Amount   : {format_currency(draft.get('amount', 0))}\n"
            f"  Type     : {draft.get('payment_type', '?')}\n"
            f"\nNo Odoo write performed. Set DRY_RUN=false to execute for real."
        )

    # Rate limit guard — shares the odoo_write bucket with invoice confirms
    rate_err = _check_rate_limit("odoo_write")
    if rate_err:
        return f"RATE LIMIT EXCEEDED: {rate_err}"
