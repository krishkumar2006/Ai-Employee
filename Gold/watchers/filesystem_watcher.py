...
# Gold Tier: path setup BEFORE pip watchdog import.
# The project root contains watchdog.py which shadows the pip `watchdog`
# package. Strip project root from sys.path first, import the pip package,
# then re-add project root so audit_logger and siblings can be found.
...
# Now safe to add project root — pip watchdog is already imported.
...
# ---------------------------------------------------------------------------
# Logging  (plain logger for console + file; AuditLogger for structured audit)
# ---------------------------------------------------------------------------
logging.basicConfig(
    level=logging.INFO,
...
audit_log  = AuditLogger("filesystem_watcher")
...
class InboxHandler(FileSystemEventHandler):
    ...
    def on_created(self, event: FileCreatedEvent) -> None:
        ...
        try:
            ...
            audit_log.exception(EV_API_FAIL, exc, source_file=file_path.name)
...
def main() -> None:
    ...
    audit_log.info(EV_START, inbox=str(INBOX_PATH), output=str(NEEDS_ACTION_PATH))
